...
export default function decorate(block) {
  ...
  const floorButtonTextRow = rows[4];
  const floorButtonLinkRow = rows[5];
  const galleryButtonTextRow = rows[6];
  const galleryButtonLinkRow = rows[7];
  const useMapInsteadRow = rows[8];
  const latitudeRow = rows[9];
  const longitudeRow = rows[10];
  const iconMapRow = rows[11];
  const isTnc = rows[12];
  const tncContent = rows[13];

  const useMap = useMapInsteadRow?.textContent.trim().toLowerCase() === "true";
  const mapData = {
    useMap,
    latitude: useMap ? parseFloat(latitudeRow?.textContent.trim()) : null,
    longitude: useMap ? parseFloat(longitudeRow?.textContent.trim()) : null,
    iconMap: useMap
      ? iconMapRow?.querySelector("picture")?.cloneNode(true)
      : null,
  };
  ...
  if (descriptionContent) {
    ...
    paragraphs.forEach((p) => {
      if (p.querySelector("a")) {
        ...
        if (titleAttr === titleAttr.toUpperCase()) {
  ...
  if (buttonData.length > 0) {
    ...
    buttonsWrapper.className = "text-image__buttons-wrapper";
  ...
  if (window.adobe && window.adobe.target) {
    adobe.target.getOffer({
      ...
      success: function (offer) {
        ...
        } else if (mapData.useMap && mapData.latitude && mapData.longitude) {
          const mapContainer = document.createElement("div");
          mapContainer.id = `map-${Math.random().toString(36).substr(2, 9)}`;
          mapContainer.className = "text-image__map-container";
          imageCol.append(mapContainer);
          import("./text-image-map.js").then(({ initMap }) => {
            initMap(
              mapContainer,
              mapData.latitude,
              mapData.longitude,
        ...
        } else if (imageRow?.querySelector("picture")) {
          imageCol.append(imageRow.querySelector("picture").cloneNode(true));
        }
        console.log(
          "Target API response:",
